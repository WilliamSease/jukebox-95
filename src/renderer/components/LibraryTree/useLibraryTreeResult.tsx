...
export function useLibraryTree(): UseLibraryTreeResult {
  ...
  const buildLibraryTree = useCallback((songs: Child[]) => {
    ...
    (async () => {
      try {
        ...
        if (cachedSongs && cachedSongs.length > 0) {
          setTree(buildLibraryTree(cachedSongs));
  ...
  const initialize = useCallback(async (newSongs: Child[]) => {
    setIsLoading(true);
    setError(null);
    try {
      const newTree = buildLibraryTree(newSongs);
      setTree(newTree);
      setCachedAt(Date.now());
      await saveLibraryCache(newSongs);
    } catch (err) {
